...
MuPDF implements this using a ``filter'' system; any stream can be filtered through a chain of processors, with the new stream written back (or not!) into the document. Each link in the chain might only do 1 job (however simple), but the whole stream can achieve complex effects.
...
The second part of the structure consists of an argument (\texttt{arg}), and a function pointer (\texttt{complete}). When the filter operation has completed, if \texttt{complete} is non-NULL, it will be called, passing a \texttt{fz\_buffer} containing the textual version of the filtered stream, and \texttt{opaque}. This function can update the buffer as required (typically by appending more operators).

The final part of the structure is a pointer to an array describing the filter chain to use; \texttt{filters} should point to the start of an array of \texttt{pdf\_filter\_factory} entries:
...
Each entry in the array should have \texttt{filter} set to a function that will create the required filter (a ``factory'' function), together with \texttt{options}, a pointer to a (filter-specific) structure holding the options for that filter.

The list is terminated by an entry with its \texttt{filter} entry set to \texttt{NULL}.
...
Thus, for a processor to be used in this manner, a \texttt{pdf\_filter\_factory\_fn}
must to be provided to create it.
