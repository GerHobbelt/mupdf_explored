...
This is the beginnings of a book on MuPDF. It is far from complete, but offers useful information as far as it goes. We offer it with the latest release of MuPDF (currently 1.12) in the hopes it will be useful.
...
Many thanks are due to Tor Andersson for creating MuPDF, to everyone who has contributed to it over the years, and to all my colleagues at Artifex Software for providing an environment in which it could grow to maturity. Particular thanks are due to Sebastian Rasmussen for patiently proof-reading the book and suggesting numerous improvements.

Finally, Helen Rogers is due an honourable mention for putting up with me.
...
MuPDF comes with a selection of devices built in, though this should not be taken as a definitive list. It is expected that other devices will be written to extend MuPDF - indeed some software built from MuPDF already includes their own devices.
...
Normally you'd expect stride to be the same as width multiplied by the number of components in the image (including alpha), but in specific instances this can usefully vary.

In order to create pixmaps that refer to specific `sub areas' of larger pixmaps, the stride value for the sub-pixmap will be larger than expected (identical to that of the original pixmap), to account for the extra pixels from each original line that are not addressed.
...
Further discussion of \texttt{fz\_image}s is deferred to \rjwref{Image} in Part 2. While it is perfectly permissible for a caller to create images, in most cases they will treat them as `black boxes' to just be passed around.
...
Rectangles can be transformed using the following function, which allows for the fact that transforms may `flip' the rectangle (i.e. that a minimum coordinate may end up as a maximum one after transformation):
...
PDF (or, equivalently, Postscript) style paths are represented using the \begin{tt}fz\_path\end{tt} structure.  A PDF path consists of a sequence of instructions describing the movement of a `pen' around a given path.
...
The `logical order' of text can be thought of as being the order in which text would be read out loud, if you were reading from a page. In many cases (such as for ePub files), this is the order in which the information is stored within the file itself. Sadly, for other formats this is not always the case.

PDF files in particular have no particular defined ordering in which text is sent - as each glyph is individually positioned on the page, files can (and do) send them in any order they feel like. While most PDF files containing European languages will tend to send text in the expected logical ordering, there is no guarantee that this will always be the case. This likelihood gets even more remote as we start to deal with right-to-left text, top to bottom text, far eastern scripts, or text in multiple different scripts or languages.

The classic case logical order may differ noticeably from rendered order is for `bidirectional' text. Even if the internal document representation is in logical order, the order in which the text will actually be displayed can be quite different. Consider for instance, some source text in Hebrew. If the individual glyphs are A,B,C,D etc, then the right-to-left nature of Hebrew text means that these will be displayed in the order `DCBA' on the page.

If however, we have conventional western (arabic) numerals on the page, interspersed within the Hebrew text, this is still written left-to-right. So A,B,C,D,1,2,3,4,E,F,G would appear as `GFE01234DCBA'.
...
Different source formats cope with this in different ways. The text strings within a PDF file have already had the layout and shaping process applied - they are literally a list of positioned glyphs to be displayed on the page. The unicode values for the original text are frequently not there at all (and when they are they require specific work to derive).

Other formats, such as EPUB, take the opposite approach, by specifying the Unicode values directly, and leaving the displaying application (i.e. MuPDF) to do the conversion to glyphs).
...
Accordingly, our \texttt{fz\_text} object represents a block of text, including font style and position, together with both unicode and glyph data (subject to the availability of the information in the original file). Wherever possible we try to provide that data in logical order, though no guarantee can be made of this.
...
To allow this, we have an extensible system to request Meta operations on a stream. The \texttt{fz\_stream\_meta} function allows such calls to be made, with a key to identify the required operation, and pointer and size parameters to identify data to be passed:
...
Images are handled by the device functions to plot images. See \rjwref{Image} for more information.
...
In all such cases MuPDF will preserve the logical ordering of the unicode entries, at the cost of possibly drawing glyphs `out of order' onto the page.
...
We may look to add convenience functions in the future, as this is likely to be desirable for JNI (and other) bindings.
...
The \texttt{name} parameter is a pointer to a (short) ASCII string describing the colorspace, \texttt{n} is the number of colorants in the space, and \texttt{flags} is the value of any required flags for the colorspace combined with logical or.
