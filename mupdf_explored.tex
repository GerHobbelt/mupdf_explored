...
In some circumstances it can be desirable to force all allocations through a set of `custom' allocators. These are defined as an \texttt{fz\_alloc\_context} structure whose address is passed in to \texttt{fz\_new\_context}. This structure must exist for the life time of the returned \texttt{fz\_context} (and any clones).
...
The \texttt{malloc}, \texttt{realloc} and \texttt{free} function pointers have essentially the same semantics as the standard \texttt{malloc}, \texttt{realloc} and \texttt{free} standard functions, with the exceptions that they take an additional initial argument - that of the \texttt{user} value specified in the  \texttt{fz\_alloc\_context}.
...
Once you have finished with an \texttt{fz\_context} (either your original one, or a `cloned' one) you can destroy it using \texttt{fz\_drop\_context}.
...
Unless otherwise specified, all MuPDF API functions can throw exceptions, and should therefore be called within an \texttt{fz\_try}\slash\texttt{fz\_always}\slash\texttt{fz\_catch} construct.
...
Reading through this code, it is fairly obvious what will happen if everything works correctly. First we'll make some walls, \texttt{w}, and a roof, \texttt{r}. Then we combine the walls and the roof, to get our house, \texttt{h}. Next we tidy up the walls and the roof, and we return the completed house to our caller.
...
This is due to an obscure part of the C specification that states that any changes to local variables made between a \texttt{setjmp} and a \texttt{longjmp} can be lost.
...
Fortunately, there is a fix for this, \texttt{fz\_var}. By calling \texttt{fz\_var(w);} we can `protect' variable \texttt{w} from such unwanted behaviour.

It's not really necessary to know how this works, but for those interested, a quick explanation. The `loss' of the value occurs because the compiler can postpone writing the value back into the storage location for the variable (or can choose to just hold it in a register). The call to \texttt{fz\_var} passes the address of the variable out of scope; this forces the compiler not to hold it in a register. Further, the compiler has no way of knowing whether any functions it call might access that location, so it needs to make sure that the variable value is written back on every function call - such as \texttt{longjmp}. Hence the variable is magically protected.
...
\begin{enumerate}
\item All MuPDF functions except those that explicitly state otherwise, throw exceptions on errors, and must therefore be called from within an \texttt{fz\_try}\slash \texttt{fz\_catch} construct.
\item A \texttt{fz\_try} block must be paired with an \texttt{fz\_catch} block, and optionally an \texttt{fz\_always} block can appear between them.
\item Never return from an \texttt{fz\_try} block.
\item An \texttt{fz\_try} block will terminate when control reaches the end of the block, or when \texttt{break} is called.
\item Any local variable that is changed within an \texttt{fz\_try} block may lose its value if an exception occurs, unless protected by \texttt{fz\_var} call.
...
As mentioned above, most MuPDF objects are reference counted. This means that on  creation (typically with an \texttt{fz\_new\_...} call), they have a reference count of 1. Think of these object pointers as `handles'.
...
As an example, an \texttt{fz\_path} structure is defined as:
...
Every ``storable piece of information'' in MuPDF is held in a data structure that begins with an \texttt{fz\_storable} structure. Rather than repeatedly say ``a storable piece of information'', we shall henceforth just say ``an \texttt{fz\_storable}''.
...
We solve this, by using an \texttt{fz\_store\_type} structure:
...
The \texttt{keep\_key} and \texttt{drop\_key} entries are used to implement reference counting of keys. Because keys can be an amalgam of several reference counted objects, the keep and drop functions provided here can take and drop these in sync.
...
Once the Store has formed a \texttt{fz\_store\_hash} it can then generate the required hash for the hashtable as required.
...
To this end, all exceptions thrown due to allocation failures have the \texttt{FZ\_ERROR\_OOM} type, enabling callers to easily distinguish them using \texttt{fz\_caught} and to react accordingly.
...
For embedded systems, or secure applications, the use of a local filing system may be inappropriate, so an alternative is available whereby documents can be opened from an \texttt{fz\_stream}. See \rjwref{Stream} for more details on \texttt{fz\_stream}s.
...
Almost any data source can be wrapped up as an \texttt{fz\_stream}; see \rjwref{Stream} for more details.
...
Some document types (such as PDF) can require passwords to allow the file to be opened. After you have obtained an \texttt{fz\_document}, you should therefore check whether it needs a password using \texttt{fz\_needs\_password}:
...
The results of the layout will depend both upon a target width and height, a given font size, the CSS styles in effect. Documents can be laid out multiple times to allow changes in these properties to take effect.
...
Annotations are reference counted and can be kept and dropped as usual. They can also be bounded, by passing a rectangle to \texttt{fz\_bound\_annot}:
...
To render a page, you first need to know how big it is. This can be discovered by calling \texttt{fz\_bound\_page}, passing an \texttt{fz\_rect} in to be populated:
...
All three of these functions (\texttt{fz\_run\_page}, \texttt{fz\_run\_page\_contents}, \texttt{fz\_run\_annot}) take an \texttt{fz\_cookie} pointer. The Cookie is a lightweight way of controlling the processing of the page. For more details, see \rjwref{Cookie}. For most simple cases this can be \texttt{NULL}.

\section{Presentations}

Some file formats, such as PDF can be used as `presentations', where pages are displayed as a slideshow for a given length of the time before automatically transitioning to the next page. Support for this is facilitated by having a routine to fetch details of what presentation details have been set for a given page:
...
We describe the convenience functions here; implementers of devices can trivially extrapolate the behaviour of the function pointers from these descriptions. For example, the \texttt{fz\_fill\_path} function described here is implemented by the \texttt{fill\_path} function pointer in the \texttt{fz\_device} that takes the identical arguments and has the same return conditions.
...
The \texttt{fz\_clip\_text} and \texttt{fz\_clip\_stroke\_text} functions are used to start a clip. Subsequent operations will be clipped through the areas delimited by these, until an \texttt{fz\_pop\_clip} is seen. See \rjwref{Clipping} for more details.
...
The \texttt{fz\_clip\_image\_mask} function is used to start a clip. Subsequent operations will be clipped through the area delimited by this, until an \texttt{fz\_pop\_clip} is seen. See \rjwref{Clipping} for more details.
...
\subsection{Clipping and Masking}
\label{Clipping}
...
\subsection{Groups and Transparency}
...
\texttt{incomplete\_ok} and \texttt{incomplete} fields are used for this. See \rjwref{ProgressiveMode}) for more details.
...
\texttt{FZ\_IGNORE\_IMAGE} being enabled implies that a device will (or should) make no effort to handle images. For example, when playing back a display list (with \texttt{fz\_run\_display\_list}), if the target device sets \texttt{FZ\_IGNORE\_IMAGE}, no image related calls will be made.

\texttt{FZ\_IGNORE\_SHADE} being enabled implies that a device will (or should) make no effort to handle shadings. For instance, if you are doing a quick pass across files trying to generate low quality thumbnail images, you may choose to disable shadings for speed.

\texttt{FZ\_DONT\_INTERPOLAGE\_IMAGES} being enabled prevents the draw device performing interpolation. MuTool Draw uses this to inhibit interpolation when anti-aliasing is disabled. Finer control over this can now be given using the Tuning Context (see \rjwref{Tuning}).
...
The \texttt{fz\_rect} passed to the \texttt{fz\_new\_bbox\_device} must obviously stay in scope for the duration of the life of the device as it will be updated on exit with the bounding box for the contents.
...
For more information about page manipulation in PDF documents, see \rjwref{PDFPageManipulation}.
...
If the device is run with \texttt{passthrough} as NULL, then as soon as it encounters a ``definite'' non-neutral colour, it will throw a \texttt{FZ\_ABORT} error.
...
Further information on Colorspaces can be found within \ref{ColorManagement}.
...
The \texttt{fz\_image} structure is used to represent a generic Image object in MuPDF. It can be viewed as an encapsulation from which both a rendering of an image (as an \texttt{fz\_pixmap}) and (often) the original source data can be retrieved.

The primary use of an \texttt{fz\_image} is to allow a rendered pixmap to be retrieved. This is done by calling:
...
The \texttt{fz\_compressed\_image} structure is a specialisation of \texttt{fz\_image}, that holds the source data for an image in an \texttt{fz\_compressed\_buffer}. This is the usual form for images created from PDF and XPS files.
...
If the supplied \texttt{fz\_image} is not an \texttt{fz\_compressed\_image} then it will return \texttt{NULL}.

\subsection{Pixmap Images}

The \texttt{fz\_pixmap\_image} structure is a specialisation of \texttt{fz\_image}, that has an \texttt{fz\_pixmap} as its source data. This exists to allow \texttt{fz\_pixmaps} from other sources to be easily fed into the MuPDF rendering engine.
...
Postscript (or PDF) style paths are represented using the \begin{tt}fz\_path\end{tt} structure.  A postscript path consists of a sequence of instructions describing the movement of a `pen' around a given path.
...
Where filling a path simply requires details of the fill to be used, stroking a path can radically alter its appearance. The details of the stroke attributes are passed in a \begin{tt}fz\_stroke\_state\end{tt} structure.
...
To isolate callers from the implementation specifics of paths, MuPDF offers a mechanism to `walk' an \texttt{fz\_path}, getting a callback for each command in the path.
...
MuPDFs central text type is an \texttt{fz\_text} structure. The exact definition of this structure has evolved considerably in the past to accommodate the needs of different input formats, and it is possible this will continue in future. Accordingly we have hidden the implementation behind an interface.
...
Some input formats only specify glyph ids, whereas some only specify unicode values. In order to allow us to transmute one format into another, we require \texttt{fz\_text} objects to encapsulate both forms of data at the same time.
...
Accordingly, our \texttt{fz\_text} object represents a block of text in logical order, including font style and position, together with both unicode and glyph data (subject to the availability of the information in the original file).

If more information is required, then details of the current implementation are included in \rjwref{TextImplementation}, otherwise just use it as a simple black box.
...
For devices that require shadings as rasterised objects, we provide a function to paint a shading to an \texttt{fz\_pixmap}:
...
As a convenience function, MuPDF provides a mechanism for reading the entire contents of a stream into an \texttt{fz\_buffer}.
...
\section{Implementing an \texttt{fz\_stream}}
...
In our example, if the state was created by a simple \texttt{fz\_malloc\_struct(ctx, foo\_state)} then \texttt{foo\_close} might be as simple as an \texttt{fz\_free(ctx, state)}. If the internal state of the stream is more complex then the destructor will be similarly more complex.
...
\section{Implementing a \texttt{fz\_output}}
...
If <state> needs no destruction, then we can use NULL in place of \texttt{foo\_close}. Otherwise \texttt{foo\_close} should be a function of type:
...
This can be as simple as doing \texttt{fz\_free(ctx, state)}, or (depending on the complexity of the state structure) can require more involved operations to clean up.
...
The first function outputs a pixmap to a utf-8 encoded filename as an \texttt{XXX} formatted file. If the pixmap is not in a suitable colorspace/alpha configuration, then an exception will be thrown.

The second function does the same thing, but to a given \texttt{fz\_output} rather than to a named file. The use of an \texttt{fz\_output} allows for writing to memory buffers, or even potentially to encrypt or compress further as the write progresses.

The third function returns an \texttt{fz\_band\_writer} to do the same thing.
...
There are 2 sets of output functions available for PWG, those that take \texttt{fz\_pixmap}s (for contone output) and those that take \texttt{f\_bitmap}s (for halftoned output).
...
With such images, the data is held in an \texttt{fz\_compressed\_buffer}:
...
To determine if an \texttt{fz\_image} is a compressed image, call:
...
To create an image from a standard type, simply call the appropriate function. For example, if you have an \texttt{fz\_buffer} with the source data:
...
If the data cannot be recognised from its header, and more information is required, then the data can be formed in an \texttt{fz\_compressed\_buffer}, and an image created with:
...
Support for a new type of image, can be implemented fairly simply, by defining a structure derived from an \texttt{fz\_image}, perhaps:
...
MuPDF is written as an extensible framework for handling different document types. Each different document format provides an \texttt{fz\_document\_handler} structure that provides the required callbacks to recognise and open files of its supported type. For example:
...
To create an \texttt{fz\_document} use the \texttt{fz\_new\_document} macro. For a document of type foo, typically a \texttt{foo\_document} structure would be defined as below:
...
Support for PDF (Portable Document Format) is provided by \texttt{pdf\_document\_handler}. All current versions at the time of writing (i.e  up to and including PDF 1.7) are supported.
...
Once a page has been loaded, if its contents are to be `run' as normal (using e.g. \texttt{fz\_run\_page}) any error (such as failing to read a font, or an image, or even a content stream belonging to the page) will result in a rendering that aborts with an \texttt{FZ\_ERROR\_TRYLATER} error. The caller can catch this and display a placeholder instead.
...
The \texttt{fz\_buffer\_processor} is designed to produce an \texttt{fz\_buffer} from an input stream of operators. This is frequently found coupled with a \texttt{fz\_filter\_processor}, to gather up the filtered version of the operator stream ready for reinsertion into the document.
...
This solution is implemented within MuPDF as:

\begin{lstlisting}
A \texttt{pdf\_graft\_map} is a mechanism for allowing multiple objects from one file to be `grafted' into another, without unnecessary duplication of data.
/*
...
These enable the HTML agent to easily create an \texttt{fz\_image} out of an SVG. This \texttt{fz\_image} has the property that it remains scalable, and hence will not appear pixellated if the document is reflowed to different dimensions.
...
Its use of the engine is encapsulated within the \texttt{fz\_cmm\_engine} structure. Currently we provide an implementation of this structure using a modified version of LCMS2, but systems with other CMM engines in already can use those instead by reimplementing the functions therein.
...
There are applications when it can be useful to treat document formats as slideshows - a form of poor man's PowerPoint if you will.

Indeed the PDF format specification includes details of how to define the hold time, transition time and transition style within a PDF file. 
...
What it can do is to provide some functions to aid the caller in the task of querying the transitions required and generating bitmaps of various stages of common transitions.
...
\section{Transition Rendering Helper}

This helper function is largely independent of the rest of MuPDF, and will be dropped from the library if it is not used.

Given rendered pixmaps of the start/end of the transition, this will generate the required frame of the transition into the supplied target pixmap:
...
\chapter{Naming conventions and style}
...
Function should be named according to one of the following schemes:
...
\begin{itemize}
\item fz for common code
\item pdf, xps, etc., for interpreter specific code
\end{itemize}
...
\begin{itemize}
\item new, find, load, open, keep -- return objects that you are responsible for freeing.

\item drop -- relinquish ownership of the object passed in.
\end{itemize}
...
\begin{itemize}
\item lookup -- return a value or borrowed pointer
\item find -- return an object that the caller is responsible for freeing
\end{itemize}
...
\begin{itemize}
\item int is assumed to be 32bit at least.
\item short is assumed to be exactly 16 bits.
\item char is assumed to be exactly 8 bits.
\item array sizes, string lengths, and allocations are measured using size\_t.
        size\_t is 32bit in 32bit builds, and 64bit on all 64bit builds.
\item buffers of data use unsigned chars (or uint8\_t).
\item Offsets within files/streams are represented using fz\_off\_t.
        fz\_off\_t is 64bits in 64bit builds, or in 32bit builds with FZ\_LARGEFILE defined.
...
If an object is returned by a function with one of the special words that take ownership, you are responsible for freeing it by calling "drop" or "free", or "close" before you return. You may pass ownership of an owned object by returning it only if you name the function using one of the special words.
