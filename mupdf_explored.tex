...
To avoid deadlocks, MuPDF guarantees never to take lock \textit{n} if that thread already holds lock \textit{m} (for \textit{n} > \textit{m}).
...
So what happens when the failure occurs in \texttt{make\_roof} fails? Let's run through the code again.
...
\item Any variable that is changed within an \texttt{fz\_try} block may lose its value if an exception occurs, unless protected by \texttt{fz\_var} call.
...
When any given document handler is told to run the page (\texttt{fz\_run\_page}) the appropriate document interpreter serialises the page contents as a series of graphical operations, and calls the device interface to perform these actions.
...
In order to represent a given color for a graphical object, we need both the color component values and details of the colorspace that the color is specified in. Color values are defined simply as floats (between 0 and 1 inclusive), and colorspaces are defined using the \texttt{fz\_colorspace} structure.
...
Various functions exist to either save pixmaps to files, or to (more generally) write them to \texttt{fz\_output} streams.

In general, these functions fall into 3 categories of increasing complexity.

Firstly, functions of the form \texttt{fz\_save\_pixmap\_to\_}... take a pixmap and write it to a given filename in the local filesystem.

Secondly, functions of the form \texttt{fz\_write\_pixmap\_to\_}... take a pixmap and write it to a given \texttt{fz\_output}. The use of an \texttt{fz\_output} allows for writing to memory buffers, or even potentially to encrypt or compress further as the write progresses.

Finally, we have an \texttt{fz\_band\_writer} class, that allows images to be written to file in `bands', thus minimising the amount of memory required at any one time. Typically a band writer is created using a call such as \texttt{fz\_new\_png\_band\_writer}:
...
The second function does the same thing, but to a given \texttt{fz\_output} rather than to a named file.
...
Having created a \texttt{fz\_band\_writer} using one of the creation functions defined in the following sections, the page output starts by calling \texttt{fz\_write\_header}. This both configures the band writer for the type of data that is being sent, and triggers the output of the file header.
...
Next, the caller should render bands of the page in turn, and pass them in to \texttt{fz\_write\_band}.
...
At this point, the caller can either call \texttt{fz\_write\_header} to start a new page, or they can call \texttt{fz\_drop\_band\_writer} to clean up.
